...
export default function Home() {
  ...
  const { ref: emoji, inView: emojisecisvisible } = useInView();
  return (
    <Layout>
      <Head>
      ...
      <Section backgroundColor="secondary">
        <Container>
          <h2 className={styles.heading} ref={emoji}>
            Have you scrolled down here yet? 
            <span className={`${styles.emoji} ${emojisecisvisible ? styles.animateEmoji : ''}`}>🧐</span>
          </h2>

          <p ref={magic}>
            IDK 
            <span className={`${styles.man} ${magicSection ? styles.animateMan:''}`}>🤷‍♂️</span></p>
        </Container>
      </Section>
